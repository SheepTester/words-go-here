module Good exposing (UtilityDatum(..), Calculation, Good)
...
type alias CalculateDatumModel = (Maybe Utility, List Calculation)
...
type alias MaxUtilityModel = (Price, List Int)
...
maxUtilityStep goods (income, purchased) =
    let
        withMUPP = List.map2 Tuple.pair purchased goods
    ...
    in
        case (maxMUPP, spent <= income) of
            (Just max, True) ->
                withMUPP
                    |> List.map (\((count, _), mupp) -> if justEqual mupp maxMUPP then
                        count + 1
                    else
                        count)
...
maxUtility goodData income =
    let
        goods = goodData |> Array.toList |> List.map resolveGood
    in
